...
import type { LogisticRequest } from '../../shared/types/Order';
...
export function OrderDetails() {
  ...
  const handleUpdateServiceQuantity = async (serviceId: number, quantity: number) => {
    if (!id || quantity < 1) return;
    await dispatch(updateServiceInOrder({ orderId: Number(id), serviceId, quantity }));
  };
