...
func (r *Repository) GetServicesWithFilters(search string, minPrice, maxPrice *float64, dateFrom, dateTo *time.Time) ([]ds.Service, error) {
	var TransportService []ds.Service
...
func (r *Repository) DeleteOrder(orderID int) error {
	// Каскадное удаление автоматически удалит связанные записи в LogisticRequestService
...
func (r *Repository) AddToCart(serviceID int) error {
	...
	// upsert в LogisticRequestService
	...
	_, err = sqlDB.Exec(`
		INSERT INTO "LogisticRequestService"(order_id, service_id, quantity)
        VALUES ($1, $2, 1)
        ON CONFLICT (order_id, service_id)
		DO UPDATE SET quantity = "LogisticRequestService".quantity + 1
    `, orderID, serviceID)
	return err
}
...
func (r *Repository) RemoveFromCart(serviceID int) error {
	...
	var qty int
	err = sqlDB.QueryRow(`SELECT quantity FROM "LogisticRequestService" WHERE order_id=$1 AND service_id=$2`, orderID, serviceID).Scan(&qty)
	...
	if qty > 1 {
		_, err = sqlDB.Exec(`UPDATE "LogisticRequestService" SET quantity = quantity - 1 WHERE order_id=$1 AND service_id=$2`, orderID, serviceID)
	} else {
		_, err = sqlDB.Exec(`DELETE FROM "LogisticRequestService" WHERE order_id=$1 AND service_id=$2`, orderID, serviceID)
	}
	return err
}
...
func (r *Repository) GetCartServices() ([]ds.Service, error) {
	...
	for _, it := range items {
		...
		if err == nil {
			TransportService = append(TransportService, s)
		}
	}
	return TransportService, nil
}
...
func (r *Repository) GetCartCount() int {
	...
	_ = sqlDB.QueryRow(`SELECT COALESCE(SUM(quantity),0) FROM "LogisticRequestService" WHERE order_id=$1`, orderID).Scan(&count)
	if count.Valid {
