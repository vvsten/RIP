...
func registerRoutes(r *gin.Engine, handler *handler.Handler) {
    ...
    r.GET("/api/users/profile", handler.GetUserProfile)
    r.PUT("/api/users/profile", handler.UpdateUserProfile)
    r.POST("/api/users/login", handler.LoginUser)
    r.POST("/api/users/logout", handler.LogoutUser)
    ...
    // М-М заявка-услуга
    r.POST("/api/orders/services", handler.AddServiceToOrder)
    r.DELETE("/api/orders/services", handler.RemoveServiceFromOrder)
    r.PUT("/api/orders/services", handler.UpdateOrderService)
