...
func (h *Handler) GetCart(ctx *gin.Context) {
	...
	if err != nil {
		ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Ошибка получения услуг"})
		return
	}
...
func (h *Handler) CalculateService(ctx *gin.Context) {
	...
	if err := ctx.ShouldBindJSON(&request); err != nil {
		if err := ctx.ShouldBind(&request); err != nil {
			ctx.JSON(http.StatusBadRequest, gin.H{"error": "Неверные данные запроса"})
			return
		}
	}
	...
	if err != nil {
		ctx.JSON(http.StatusNotFound, gin.H{"error": "Тип транспорта не найден"})
		return
	}
	...
	// Проверяем ограничения
	if request.Weight <= service.MaxWeight && volume <= service.MaxVolume {
		// Простая формула расчета расстояния между городами
		distance := calculateDistance(request.FromCity, request.ToCity)
		...
		// Учитываем вес (рублей за кг)
		weightCost := request.Weight * 0.1 // 10 копеек за кг
		totalCost += weightCost
...
func (h *Handler) SubmitOrder(ctx *gin.Context) {
	...
	if len(request.Services) == 0 {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Нет типов транспорта для оформления заявки"})
		return
	}
	...
	ctx.JSON(http.StatusOK, gin.H{
		...
		"order_id": 12345, // Временный ID заявки
	})
}
...
func (h *Handler) SearchTransport(ctx *gin.Context) {
	...
	if err != nil {
		logrus.Error(err)
		if ctx.GetHeader("Content-Type") == "application/json" {
			ctx.JSON(http.StatusInternalServerError, gin.H{"error": "Ошибка поиска транспорта"})
		} else {
			ctx.HTML(http.StatusInternalServerError, "error.html", gin.H{
