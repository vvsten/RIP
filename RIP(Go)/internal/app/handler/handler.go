...
func (h *Handler) RemoveFromCart(ctx *gin.Context) {
	...
	// Удаляем всю заявку
	err = h.Repository.DeleteOrder(orderID)
	if err != nil {
		fail(ctx, http.StatusInternalServerError, "ошибка удаления заявки")
		return
	}

	// Создаём новую пустую корзину для пользователя
	creatorID := ds.GetCreatorID()
	_, err = h.Repository.CreateDraftOrder(creatorID)
	if err != nil {
		// Логируем ошибку, но не возвращаем её пользователю
		logrus.Error("failed to create new draft order:", err)
	}

	ctx.JSON(http.StatusOK, gin.H{
		"success": true,
		"count":   0,
		"message": "Заявка удалена, создана новая корзина",
	})
}
...
func (h *Handler) RegisterUser(ctx *gin.Context) {
    ...
    if err := h.Repository.CreateUser(&user); err != nil {
        fail(ctx, http.StatusBadRequest, "user already exists")
        return
    }
...
func (h *Handler) LogoutUser(ctx *gin.Context) {
    ctx.JSON(http.StatusOK, gin.H{"status": "ok", "message": "logged out"})
}
...
func (h *Handler) FormOrder(ctx *gin.Context) {
    ...
    err = h.Repository.FormOrder(id)
    if err != nil {
        fail(ctx, http.StatusBadRequest, err.Error())
        return
    }

    ctx.JSON(http.StatusOK, gin.H{"status": "ok", "message": "order formed successfully"})
}
...
func (h *Handler) CompleteOrder(ctx *gin.Context) {
    ...
    moderatorID := ds.GetModeratorID()
    err = h.Repository.CompleteOrder(id, req.Status, moderatorID)
    if err != nil {
        fail(ctx, http.StatusBadRequest, err.Error())
        return
    }

    ctx.JSON(http.StatusOK, gin.H{"status": "ok", "message": "order completed successfully"})
}
...
func (h *Handler) RemoveServiceFromOrder(ctx *gin.Context) {
    ...
    err := h.Repository.RemoveServiceFromOrder(req.OrderID, req.ServiceID)
    if err != nil {
        fail(ctx, http.StatusBadRequest, err.Error())
        return
    }
...
func (h *Handler) UpdateOrderService(ctx *gin.Context) {
    ...
    err := h.Repository.UpdateOrderService(req.OrderID, req.ServiceID, req.Quantity, req.Order, req.Comment)
    if err != nil {
        fail(ctx, http.StatusBadRequest, err.Error())
        return
    }
