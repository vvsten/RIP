...
// FormOrder - формирование заявки (проверка обязательных полей)
func (r *Repository) FormOrder(orderID int) error {
    var order ds.Order
    ...
    if order.FromCity == "" || order.ToCity == "" || order.Weight <= 0 {
        return fmt.Errorf("не заполнены обязательные поля: города и вес груза")
    }
